...
def group_mungos_by_system(page_mungos, score_img=None, page_num=None,
                           MIN_PEAK_WIDTH=5,
                           NONWHITE_COLUMNS_TOLERANCE=0.001):
    """(DEPRECATED) Use ``group_mungos_by_system_paths()``.
    ...
    :returns: ``(system_bboxes, system_mungos)`` where ``system_bboxes``
        ...
        objects
    """
    logging.warn('Deprecated: use group_mungos_by_system_paths() instead.')
    ...
    system_mungos = group_mungos_by_bbox(page_mungos, system_regions)
    ...
    merged_system_mungos = group_mungos_by_bbox(page_mungos,
                                                merged_system_boundaries)

    return merged_system_boundaries, merged_system_mungos


def group_mungos_by_bbox(page_mungos, bounding_boxes):
    """Group MuNG objects based on which region's bounding box they overlap."""
    system_mungos = [[] for _ in bounding_boxes]
    # TODO
    # For each MuNG object, find the closest region
    for i, (t, l, b, r) in enumerate(bounding_boxes):
